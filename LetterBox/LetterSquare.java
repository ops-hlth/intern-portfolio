...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        String wordAt = words[wordNum]; 
        if (charNum == 0){
            if (wordNum == 0){
                return true; 
            }
            else {
                String sliceLast = lastLetter(this.words[wordNum - 1]);
                if (sliceLast.equals(letter)){
                    return true; 
                }   
            }
        }
        else {
            if (!this.alreadyUsed(wordAt + letter)){
                if (this.onSameSide(letter, lastLetter(wordAt)) == false){
                    if (dictionary.hasString(wordAt + letter)){
                        return true; 
                    }
                }   
            }
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        if (this.allLettersUsed() && dictionary.hasFullWord(this.words[wordNum]) && this.words[wordNum].length() >= 3){ 
        ...
        for (int i = 0; i < this.letters.length; i++){
            if (this.isValid(this.letters[i], wordNum, charNum)){
                this.addLetter(this.letters[i], wordNum);
                if (this.solveRB(wordNum, charNum + 1, maxWords)){
                return true; 
                }
                else { 
                if ((this.words[wordNum].length() >= 3) && (dictionary.hasFullWord(words[wordNum]))){
                    this.solveRB(wordNum + 1, 0, maxWords); 
                    if (this.allLettersUsed()){
                        return true; 
                    }
                }
            }
